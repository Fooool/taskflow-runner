...
def run(src, dest, silent=False, **kwargs):
    """Copy files to a destination directory or rename them in the process."""
    ...
    if len(files) > 1 and not os.path.isdir(dest):
        print(
            f"❌ Error: Destination '{dest}' must be a directory when copying multiple files.")
        return False
    ...
    for src_file in files:
        if os.path.isdir(dest):
            dest_path = os.path.join(dest, os.path.basename(
                src_file))  # Copy into directory
        else:
            dest_path = dest  # Rename file while copying

        try:
            shutil.copy(src_file, dest_path)
            if not silent:
                print(f"✔️ Copied '{src_file}' → '{dest_path}'")
